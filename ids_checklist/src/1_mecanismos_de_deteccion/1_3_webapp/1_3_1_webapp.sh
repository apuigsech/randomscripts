...
TEST_NAME="1.3.1. Ataques comunes sobre aplicaciones web"
TEST_VERSION="1.0"
...
function run_test () {
	local target_url=$1
	local base_cmd="${CURL_BIN} --insecure"

	for filename in $(ls ${SAMPLES_DIR}); do
		write_log "TASK START ${filename}"
		 for sample in $(cat ${SAMPLES_DIR}/${filename}); do
		 	exec_task "${base_cmd} '${target_url}/${sample}'"
		 	exec_task "${base_cmd} '${target_url}/p=${sample}'"
		 	exec_task "${base_cmd} '${target_url}' --data '${sample}'"
		 	exec_task "${base_cmd} '${target_url}' --data 'p=${sample}'"
		 	exec_task "${base_cmd} '${target_url}' --cookie 'p=${sample};'"
		 done
